...
percent_thresh <- unlist(strsplit(args[5],","))
libdir <- args[6]
...
for (perc in c(percent_thresh)) {
  data.pipe<-function(dfm,dfu,dfe,dfre,cut){
    ...
    met<-dfm
    met<-subset.data.frame(met, select=-c(genus,family,order,class,kingdom,domain))
    ...
    reform<-subset(reform, reform$taxid %in% bxD$species)
    for (r in c(2,3,4,6)) { 
  ...
  uniq_box<-function(df, dfu){
    ...
    library(dplyr)
    #chop off taxa names
    uniq <- subset(dfu, select=-c(genus,family,order,class,kingdom,domain))
    ...
    htmlwidgets::saveWidget(as_widget(u), paste("species_level_unique_reads_",perc,"perc.html",sep=""), selfcontained=TRUE)
  }
  uniq_box(df, dfu)
  ...
  mean_box<-function(df, dfm){
    ...
    library(dplyr)
    #chop off taxa names
    mean <- subset(dfm, select=-c(genus,family,order,class,kingdom,domain))
    ...
    htmlwidgets::saveWidget(as_widget(u), paste("species_level_mean_reads_",perc,"perc.html",sep=""), selfcontained=TRUE)
  }
  mean_box(df, dfm)
  ...
  error_box<-function(df, dfe){
    ...
    library(dplyr)
    #chop off taxa names
    error <- subset(dfe, select=-c(genus,family,order,class,kingdom,domain))
    ...
    htmlwidgets::saveWidget(as_widget(u), paste("species_level_errors_",perc,"perc.html",sep=""), selfcontained=TRUE)
  }
  error_box(df, dfe)
  ...
  rel_error_box<-function(df, dfre){
    ...
    htmlwidgets::saveWidget(as_widget(u), paste("species_level_rel_errors_",perc,"perc.html",sep=""), selfcontained=TRUE)
  }
