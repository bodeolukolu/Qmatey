...
species1 <- species$species; length(x1) <- n; species2 <- species$genus; length(species2) <- n; species <- cbind(species1,species2)
...
plot <- ggplot(ctv,aes(x=Cross_taxon_validation,y=value,group=variable,color=variable))+
  ...
  geom_point()+xlab("Cross Taxon Comparison")+
  ylab("Value")+ theme_bw() + theme(legend.position="bottom") + 
  theme(text = element_text(size = 20),axis.text.x = element_text(angle=20,size=18.5,hjust = 1)) + labs(color='Metric') +
  theme(plot.margin = margin(2,2,2,2, "cm"))
