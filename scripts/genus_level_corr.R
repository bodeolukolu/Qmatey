...
write.table(corr,paste("Metagenome_",perc,"perc_genus_corr.txt",sep=""), sep="\t",row.names=FALSE, col.names=FALSE, quote = F)
...
for (minc in (as.numeric(mincorr))) {
  for (maxc in (as.numeric(maxcorr))) {
    ...
    ggsave(filename=paste("Metagenome_genus_",perc,"perc_","neg",maxc,"_pos",minc,"_corr.tiff",sep=""), plot=plot, width=35, height=35, dpi=600, compression = "lzw", limitsize = FALSE)
  }
}
