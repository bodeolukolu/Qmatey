...
dfm$percent <- (rowSums(dfm[,2:(ncol(dfm)-4)] > "0")/(ncol(dfm)-5))*100
...
data.pipe<-function(dfm,dfrm,dfu,dfe,dfre,cut){
  ...
  met<-dfm
  met<-subset.data.frame(met, select=-c(class,kingdom,domain))
  ...
  reform<-subset(reform, reform$taxid %in% bxD$order)
  for (r in c(2,3,4,6)) { 
...
uniq_box<-function(df, dfu){
  #chop off order names
  uniq <- subset(dfu, select=-c(class,kingdom,domain))
  ...
  htmlwidgets::saveWidget(as_widget(u), paste("order_level_unique_reads_",perc,"perc.html",sep=""), selfcontained=FALSE)
}
uniq_box(df, dfu)

mean_box<-function(df, dfm){
  #chop off order names
  mean <- subset(dfm, select=-c(class,kingdom,domain))
  ...
  htmlwidgets::saveWidget(as_widget(u), paste("order_level_mean_reads_",perc,"perc.html",sep=""), selfcontained=FALSE)
}
mean_box(df, dfm)

mean_box<-function(df, dfrm){
  #chop off order names
  mean <- subset(dfrm, select=-c(class,kingdom,domain))
  ...
  htmlwidgets::saveWidget(as_widget(u), paste("order_level_rel_mean_reads_",perc,"perc.html",sep=""), selfcontained=FALSE)
}
mean_box(df, dfrm)
...
error_box<-function(df, dfe){
  #chop off order names
  error <- subset(dfe, select=-c(class,kingdom,domain))
  ...
  htmlwidgets::saveWidget(as_widget(u), paste("order_level_errors_",perc,"perc.html",sep=""), selfcontained=FALSE)
}
error_box(df, dfe)

rel_error_box<-function(df, dfre){
  #chop off order names
  rel_error <- subset(dfe, select=-c(class,kingdom,domain))
  ...
  htmlwidgets::saveWidget(as_widget(u), paste("order_level_rel_errors_",perc,"perc.html",sep=""), selfcontained=FALSE)
}
