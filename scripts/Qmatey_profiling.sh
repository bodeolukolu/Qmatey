...
if [[ "$blast_location" == "remote" ]]; then
	...
	if test -f $file; then
		...
		${Qmatey_dir}/tools/ncbi-blast-2.13.0+/bin/blastn -task megablast -query <(zcat combined_compressed_metagenomes.fasta.gz) -db "${remote_db}" -perc_identity 95  -max_target_seqs 1000000 \
