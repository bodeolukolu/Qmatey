...
uniq_box<-function(df, dfu){
  ...
  htmlwidgets::saveWidget(as_widget(u), paste("strain_level_unique_reads_",perc,"perc.html",sep=""), selfcontained=FALSE)
}
uniq_box(df, dfu)
...
mean_box<-function(df, dfrm){
  #chop off taxa names
  mean <- subset(dfrm, select=-c(tax_id,species,genus,family,order,class,phylum,kingdom,domain))
  fence<-data.frame(matrix(nrow=nrow(mean), ncol = 2))
  taxa<-c()
  up<-c()
  for (pl in 1:nrow(mean)) {
...
error_box<-function(df, dfe){
  ...
  htmlwidgets::saveWidget(as_widget(u), paste("strain_level_errors_",perc,"perc.html",sep=""), selfcontained=FALSE)
}
error_box(df, dfe)
...
rel_error_box<-function(df, dfre){
  ...
  htmlwidgets::saveWidget(as_widget(u), paste("strain_level_rel_errors_",perc,"perc.html",sep=""), selfcontained=FALSE)
}
