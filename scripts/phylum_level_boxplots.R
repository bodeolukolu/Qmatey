...
for (perc in c(percent_thresh)) {
  ...
  uniq_box<-function(df, dfu){
    ...
    htmlwidgets::saveWidget(as_widget(u), paste("phylum_level_unique_reads_",perc,"perc.html",sep=""), selfcontained=TRUE)
  }
  uniq_box(df, dfu)
  ...
  mean_box<-function(df, dfm){
    ...
    htmlwidgets::saveWidget(as_widget(u), paste("phylum_level_mean_reads_",perc,"perc.html",sep=""), selfcontained=TRUE)
  }
  mean_box(df, dfm)
  ...
  error_box<-function(df, dfe){
    ...
    htmlwidgets::saveWidget(as_widget(u), paste("phylum_level_errors_",perc,"perc.html",sep=""), selfcontained=TRUE)
  }
  error_box(df, dfe)
  ...
  rel_error_box<-function(df, dfre){
    ...
    xlim<-max(fence$Upper)
    reform$Phylum[is.na(reform$Phylum)] = "Unknown"
    u<-plot_ly(reform, x = ~errors, y = ~taxid, color = ~Phylum, type = "box", boxpoints = FALSE)%>%layout(title= "Boxplot of Standard Relative Error (% RSE)", xaxis = list(titlerange=c(0,xlim), title = "Relative Standard Error (% RSE)"), yaxis = list(size = 1, title = "Taxaname"))
    htmlwidgets::saveWidget(as_widget(u), paste("phylum_level_rel_errors_",perc,"perc.html",sep=""), selfcontained=TRUE)
  }
