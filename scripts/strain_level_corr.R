...
for (i in c(percent)) {
      ...
      write.table(corr,paste("Metagenome_",i,"perc_strain_corr_spearman.txt",sep=""), sep="\t",row.names=FALSE, col.names=FALSE, quote = F)
      ...
      plot <- ggplot(corr, aes(x=Var1, y=Var2, fill=coeff, size= circle_size)) +
        ...
        labs(title= paste("Strain-Level Correlogram (",i,"% missingness threshold, ",ntaxa," taxa)",sep=""))
      ggsave(filename=paste("Metagenome_strain_",i,"perc_","neg",maxc,"_pos",minc,"_corr_spearman.tiff",sep=""), plot=plot, width=35, height=35, dpi=600, compression = "lzw", limitsize = FALSE)
    }
