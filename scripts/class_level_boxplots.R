...
dfm$percent <- (rowSums(dfm[,2:(ncol(dfm)-3)] > "0")/(ncol(dfm)-4))*100 
...
mean_box<-function(df, dfrm){
  ...
  htmlwidgets::saveWidget(as_widget(u), paste("class_level_rel_mean_reads_",perc,"perc.html",sep=""), selfcontained=FALSE)
}
mean_box(df, dfrm)
