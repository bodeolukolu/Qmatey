...
for (fdr in c(TRUE,FALSE)){
  ...
  write.table(corr,paste("./",dirdes,"/Metagenome_",perc,"perc_phylum_corr_spearman.txt",sep=""), sep="\t",row.names=FALSE, col.names=FALSE, quote = F)
  ...
  for (minc in (as.numeric(mincorr))) {
    for (maxc in (as.numeric(maxcorr))) {
      ...
      ggsave(filename=paste("./",dirdes,"/Metagenome_phylum_",perc,"perc_","neg",maxc,"_pos",minc,"_corr_spearman.tiff",sep=""), plot=plot, width=35, height=35, dpi=600, compression = "lzw", limitsize = FALSE)
    }
  }
}
