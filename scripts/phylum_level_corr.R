...
if (args[5] == "true"){
  ...
  write.table(corr,paste("Metagenome_",perc,"perc_phylum_cclasso_corr.txt",sep=""), sep="\t",row.names=FALSE, col.names=TRUE, quote = F)
  ...
  for (minc in (as.numeric(mincorr))) {
    for (maxc in (as.numeric(maxcorr))) {
      ...
      ggsave(filename=paste("Metagenome_phylum_",perc,"perc_","neg",maxc,"_pos",minc,"_cclasso_corr.tiff",sep=""), plot=plot, width=35, height=35, dpi=600, compression = "lzw", limitsize = FALSE)
    }
  }
}
...
if (args[6] == "true"){
  ...
  corr <- cbind(metag_corr1, metag_pmat1)
  corr <- subset(corr, select=-c(4:5))
  write.table(corr,paste("Metagenome_",perc,"perc_phylum_spearman_corr.txt",sep=""), sep="\t",row.names=FALSE, col.names=TRUE, quote = F)
    ...
    for (maxc in (as.numeric(maxcorr))) {
      ...
      plot <- ggplot(corr, aes(x=Var1, y=Var2, fill=coeff, size= circle_size)) +
        ...
        theme(axis.text.x=element_text(size=axis_density, angle=45, vjust=1, hjust=1, margin=margin(0,0,0,0)),
              ...
              plot.title = element_text(size=axis_density)) +
        labs(title= paste("Phylum-Level Correlogram (",perc,"% missingness threshold, ",ntaxa," taxa)",sep=""))
      ggsave(filename=paste("Metagenome_phylum_",perc,"perc_","neg",maxc,"_pos",minc,"_spearman_corr.tiff",sep=""), plot=plot, width=35, height=35, dpi=600, compression = "lzw", limitsize = FALSE)
    }
  }
}
