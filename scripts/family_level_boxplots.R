...
dfm$percent <- (rowSums(dfm[,2:(ncol(dfm)-5)] > "0")/(ncol(dfm)-6))*100 
...
data.pipe<-function(dfm,dfrm,dfu,dfe,dfre,cut){
  ...
  vDrop<-c("order","class","kingdom","domain")
  mirror<-dfm[,(which(colnames(dfm) %!in% vDrop))]
  mirror$family<-as.character(mirror$family)
  mirror$phylum = as.character(mirror$phylum)
  if (colnames(mirror)[1] == "family") {
  ...
  met<-dfm
  met<-subset.data.frame(met, select=-c(order,class,kingdom,domain))
  ...
  reform<-subset(reform, reform$taxid %in% bxD$family)
  for (r in c(2,3,4,6)) { 
...
uniq_box<-function(df, dfu){
  #chop off taxa names
  uniq <- subset(dfu, select=-c(order,class,kingdom,domain))
  ...
  htmlwidgets::saveWidget(as_widget(u), paste("family_level_unique_reads_",perc,"perc.html",sep=""), selfcontained=FALSE)
}
uniq_box(df, dfu)
...
mean_box<-function(df, dfm){
  ...
  htmlwidgets::saveWidget(as_widget(u), paste("family_level_mean_reads_",perc,"perc.html",sep=""), selfcontained=FALSE)
}
mean_box(df, dfm)

mean_box<-function(df, dfrm){
  #chop off family names
  mean <- subset(dfrm, select=-c(order,class,kingdom,domain))
  ...
  xlim<-max(fence$Upper)
  reform$Phylum[is.na(reform$Phylum)] = "Unknown"
  u<-plot_ly(reform, x = ~`covMean`, y = ~taxid, color = ~Phylum, type = "box", boxpoints = FALSE)%>%layout(title = paste("Boxplot of Standard Error (",remtaxa," taxa)",sep=""), xaxis = list(range=c(0,xlim), title = "Mean Reads"), yaxis = list(size = 1, title = "Taxaname"))
  htmlwidgets::saveWidget(as_widget(u), paste("family_level_rel_mean_reads_",perc,"perc.html",sep=""), selfcontained=FALSE)
}
mean_box(df, dfrm)
...
error_box<-function(df, dfe){
  #chop off taxa names
  error <- subset(dfe, select=-c(order,class,kingdom,domain))
  ...
  htmlwidgets::saveWidget(as_widget(u), paste("family_level_errors_",perc,"perc.html",sep=""), selfcontained=FALSE)
}
error_box(df, dfe)

rel_error_box<-function(df, dfre){
  #chop off taxa names
  rel_error <- subset(dfe, select=-c(order,class,kingdom,domain))
  ...
  htmlwidgets::saveWidget(as_widget(u), paste("family_level_rel_errors_",perc,"perc.html",sep=""), selfcontained=FALSE)
}
