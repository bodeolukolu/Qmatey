...
# cd ${local_db%/*}
# perl ${Qmatey_dir}/tools/ncbi-blast-2.11.0+/bin/update_blastdb.pl --passive --decompress nt
# wait
#
# cd ${Qmatey_dir}
# taxids=$( echo $taxids | awk '{gsub(/,/," ")}1' )
# for get_species_taxids in ${taxids[@]}; do
# ./tools/ncbi-blast-2.11.0+/bin/get_species_taxids.sh -t ${get_species_taxids} > ./misc/${get_species_taxids}.txids
# done
#
...
# 	else
# 		${Qmatey_dir}/tools/ncbi-blast-2.11.0+/bin/blastn -task megablast -query combined_compressed_metagenomes.fasta -db ${local_db} -num_threads $threads -perc_identity 95 -outfmt \
...
# 			taxid_list=${Qmatey_dir}/misc/${taxid}.txids
...
# 		echo -e "${YELLOW}- Skipping BLAST and filtering hits based on defined parameters"
# 	else
# 		${Qmatey_dir}/tools/ncbi-blast-2.11.0+/bin/blastn -task megablast -query combined_compressed_metagenomes.fasta -db $remote_db -perc_identity 95 -outfmt \
# 		"6 qseqid sseqid length mismatch evalue pident gapopen qseq sseq staxids stitle" \
...
# 		echo -e "${YELLOW}- Skipping BLAST and filtering hits based on defined parameters"
# 	else
# 		${Qmatey_dir}/tools/ncbi-blast-2.11.0+/bin/blastn -task megablast -query combined_compressed_metagenomes.fasta -db ${projdir} -num_threads $threads -perc_identity 95 -outfmt \
# 		"6 qseqid sseqid length mismatch evalue pident gapopen qseq sseq staxids stitle" \
# 		-out ../alignment/combined_compressed.megablast
# 	fi
# 	wait
		...
		for ccf in $(ls * | sort -V); do
			...
			for sub in $(ls subfile* | sort -V); do (
				if [[ "$taxids" == true ]]; then
					...
					-taxidlist ${projdir}/metagenome/All.txids -outfmt "6 qseqid sseqid length mismatch evalue pident gapopen qseq sseq staxids stitle" -out ${sub}_out.blast &&
					wait
				else
					${Qmatey_dir}/tools/ncbi-blast-2.13.0+/bin/blastn -task megablast -query $sub -db "${local_db}" -num_threads 1 -perc_identity $percid -max_target_seqs 1000000 \
					-outfmt "6 qseqid sseqid length mismatch evalue pident gapopen qseq sseq staxids stitle" -out ${sub}_out.blast &&
					wait
				fi
				wait
				gzip ${sub}_out.blast &&
...
if test -f $file; then
	...
	Rscript ${Qmatey_dir}/scripts/strain_level_corr.R $strain_level_mean $min_percent_sample "${Qmatey_dir}/tools/R" &>/dev/null
...
if find ../sighits/sighits_species/ -mindepth 1 | read; then
	...
	for i in $(ls -S *_haplotig.megablast);do (
		awk '$6>=90' $i | awk 'gsub(" ","_",$0)' | awk 'BEGIN{OFS="\t"}{gsub(/-/,"\t",$1); print}' | \
...
if test -f $file; then
	...
	Rscript ${Qmatey_dir}/scripts/species_level_corr.R $species_level_mean $min_percent_sample "${Qmatey_dir}/tools/R" &>/dev/null
...
if test -f $file; then
	...
	Rscript ${Qmatey_dir}/scripts/genus_level_corr.R $genus_level_mean $min_percent_sample "${Qmatey_dir}/tools/R" &>/dev/null
...
if test -f $file; then
	...
	Rscript ${Qmatey_dir}/scripts/family_level_corr.R $family_level_mean $min_percent_sample "${Qmatey_dir}/tools/R" &>/dev/null
...
if test -f $file; then
	...
	Rscript ${Qmatey_dir}/scripts/order_level_corr.R $order_level_mean $min_percent_sample "${Qmatey_dir}/tools/R" &>/dev/null
...
if test -f $file; then
	...
	Rscript ${Qmatey_dir}/scripts/class_level_corr.R $class_level_mean $min_percent_sample "${Qmatey_dir}/tools/R" &>/dev/null
...
if find ../sighits/sighits_phylum/ -mindepth 1 | read; then
	...
	for i in $(ls -S *_haplotig.megablast);do (
		awk '$6>=97' $i | awk 'gsub(" ","_",$0)' | awk 'BEGIN{OFS="\t"}{gsub(/-/,"\t",$1); print}' | \
...
if test -f $file; then
	...
	Rscript ${Qmatey_dir}/scripts/phylum_level_corr.R $phylum_level_mean $min_percent_sample "${Qmatey_dir}/tools/R" &>/dev/null
